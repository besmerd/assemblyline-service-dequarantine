import hashlib
import struct


def calculate_sha256(file_path):
    """Calculate the SHA-256 hash of a file"""

    sha256_hash = hashlib.sha256()
    with open(file_path, 'rb') as fd:
        for chunk in iter(lambda: fd.read(4096), b''):
            sha256_hash.update(chunk)

    return sha256_hash.hexdigest()


def rc4_decrypt(data, key):
    """Decrypt data using the RC4 algorithm with the given key"""

    sbox, out = list(range(256)), bytearray(len(data))

    j = 0
    for i in range(256):
        j = (j + sbox[i] + key[i % len(key)]) % 256
        sbox[i], sbox[j] = sbox[j], sbox[i]

    i = j = 0
    for k, char in enumerate(data):
        i = (i + 1) % 256
        j = (j + sbox[i]) % 256
        sbox[i], sbox[j] = sbox[j], sbox[i]
        out[k] = sbox[(sbox[i] + sbox[j]) % 256] ^ char

    return out


def extract_defender(file_path):
    """Extract original file from a MS Defender quarantined file artifact"""

    key = [
        
    ]

    with open(file_path, 'rb') as fd:
        decrypted = rc4_decrypt(fd.read(), key)

    sd_len = struct.unpack_from('<I', decrypted, 0x8)[0]
    header_len = 0x28 + sd_len
    malfile_len = struct.unpack_from('<Q', decrypted, sd_len + 0x1C)[0]
    malfile = decrypted[header_len:header_len + malfile_len]

    return malfile
